...
function Products() {
  ...
  // Carga categorías e inventario una sola vez
  useEffect(() => {
    getCategoriesRequest().then(setCategories).catch(console.error);
    getInventoryRequest()
      .then((inv) => {
        const map = {};
        inv.forEach((item) => { map[item.productId] = item.quantity; });
        setInventoryMap(map);
      })
      .catch(console.error);
  }, []);
  ...
  // null = sin registro de inventario (no mostramos nada)
  // 0    = agotado
  // >0   = disponible
  const getStockInfo = (productId) => {
    if (!(productId in inventoryMap)) return null;
            ...
            {products.map((product) => {
              ...
              return (
                <div
                  ...
                  className={`group relative bg-white/5 border rounded-2xl overflow-hidden cursor-pointer transition-all duration-300
                    ${isOutOfStock
                      ? "border-white/5 opacity-70"
                      : "border-white/8 hover:border-[#FF6B35]/30"
                    }`}
                >
                  {/* Imagen */}
                  <div className="relative aspect-square bg-white/5 overflow-hidden">
                    ...
                    {isOutOfStock && (
                      <div className="absolute inset-0 bg-black/50 flex items-center justify-center">
                        <span className="bg-red-500/90 text-white text-xs font-black px-3 py-1.5 rounded-lg tracking-widest uppercase">
                          Agotado
                        </span>
                      </div>
                    )}
                  ...
                  <div className="p-4">
                    ...
                    <h3 className={`font-semibold text-lg mb-1 line-clamp-2 transition-colors ${isOutOfStock ? "text-zinc-500" : "text-white group-hover:text-[#FF8C42]"}`}>
                      {product.name}
                    </h3>
                    <p className="text-zinc-500 text-sm line-clamp-2 mb-3">{product.description}</p>
                    <div className="flex items-center justify-between">
                      <span className={`font-bold text-xl ${isOutOfStock ? "text-zinc-600" : "text-white"}`}>
                    ...
                    <button
                      ...
                      className={`w-full font-bold py-2.5 rounded-xl text-sm transition
                        ${isOutOfStock
                          ...
                          : "bg-gradient-to-r from-[#FF6B35] to-[#FFB347] hover:opacity-90 text-black"
                        }`}
                    >
