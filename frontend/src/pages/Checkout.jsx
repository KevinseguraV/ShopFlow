...
function Checkout() {
  ...
  useEffect(() => {
    getCartRequest().then(setCart).catch(console.error).finally(() => setLoading(false));
  }, []);
  ...
  const handleCheckout = async () => {
    if (!shippingAddress.trim()) { alert("Debes ingresar una dirección"); return; }
    try {
      setCreating(true);
      const payload = {
        shippingAddress,
      ...
      <div className="py-10 max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="bg-white/5 border border-white/8 rounded-3xl p-6">
            <h2 className="text-white text-2xl font-black mb-6">Dirección de envío</h2>
            <textarea
              value={shippingAddress}
              onChange={(e) => setShippingAddress(e.target.value)}
              rows={5}
              placeholder="Ingresa tu dirección..."
              className="w-full bg-black/20 border border-white/10 rounded-2xl p-4 text-white placeholder-zinc-600 outline-none focus:border-[#FF6B35]/50 resize-none transition"
            />
          </div>

          {/* Resumen */}
          <div className="bg-white/5 border border-white/8 rounded-3xl p-6 h-fit sticky top-6">
            <h2 className="text-white text-2xl font-black mb-6">Resumen</h2>
            ...
            <button
              ...
              disabled={creating}
              className="w-full bg-gradient-to-r from-[#FF6B35] to-[#FFB347] hover:opacity-90 disabled:opacity-50 transition text-black font-bold py-4 rounded-2xl text-lg"
            >
